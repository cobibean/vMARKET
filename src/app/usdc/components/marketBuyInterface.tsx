...
import { contract, tokenContract } from "@/app/usdc/constants/contracts";
...
export function MarketBuyInterface({ marketId, market }: MarketBuyInterfaceProps) {
  ...
  const checkApproval = async () => {
    ...
    try {
      ...
      // 2) Parse the amount using 6 decimals:
      const amountInUSDC = parseUnits(amount.toString(), 6);

      // Compare userAllowance with parsed amount (as BigInt)
      setBuyingStep(
        userAllowance < BigInt(amountInUSDC.toString()) ? "allowance" : "confirm"
      );
    } catch (error) {
      console.error(error);
    }
