...
// 4) File paths
const gamesDir = resolve('./src/app/vesta/games/NBA');
// Here's where we store Market <-> Game mapping
const mappingFilePath = resolve('./src/app/vesta/mappings/marketMappingNBA.json');
...
async function saveMarketMapping(mappingObject) {
  ...
  console.log(`marketMappingNBA.json updated. Total markets in file: ${Object.keys(mappingObject).length}`);
}
...
//-------------------------------------
const filesToProcess = ["games-2025-01-09.json"];
